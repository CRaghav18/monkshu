...
// const APP_CONSTANTS = require("./constant.js");
const DB_CREATION_SQLS = [
    //  ATTACH '${APP_CONSTANTS.DB_DIR}/teleworkrapp.db' as ${APP_CONSTANTS.ALIAS_TWDB},
    //  ATTACH '${APP_CONSTANTS.DB_DIR}/loginapp.db' as ${APP_CONSTANTS.ALIAS_LADB}
];

async function _initDB(dbPath) {
    return new Promise((resolve, reject) => {
        let dbInstance = new sqlite3.Database(dbPath, sqlite3.OPEN_READWRITE, async err => {
            if (err) {
                console.log(`Error initializing dbInstance: ${err.message}`);
                reject(err);
            } else {
...
async function runQuery(query, params = [], dbPath) {
    let dbInstance;
    try {
        dbInstance = await _initDB(dbPath);
    } catch (error) {
...
async function getQuery(query, params = [], dbPath) {
    ...
    return new Promise(async resolve => {
        dbInstance.get(query, params, async (error, row) => {
            await _closeDB(dbInstance);
            if (error) {
                console.log(`Error in dbInstance.get: ${error.message}`);
            }
            if (error || !row) {
                resolve({ result: false, error });
            } else {
                resolve({ result: true, row });
            }
        });
    });
}
...
async function getAllQuery(query, params = [], dbPath) {
    ...
    return new Promise(async resolve => {
        dbInstance.all(query, params, async (error, rows) => {
            await _closeDB(dbInstance);
            if (error) {
                console.log(`Error in dbInstance.all: ${error.message}`);
                resolve({ result: false, error });
            } else {
